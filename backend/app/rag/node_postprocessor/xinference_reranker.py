...
class XinferenceRerank(BaseNodePostprocessor):
    ...
    def __init__(
        ...
        api_key: str = "",
        api_url: str = "http://localhost:9997/v1/rerank",
    ):
    ...
    def _postprocess_nodes(
        ...
        with self.callback_manager.event(
            ...
            results = zip(range(len(nodes)), resp["results"])
            results = sorted(results, key=lambda x: x[1]["relevance_score"], reverse=True)[: self.top_n]

            new_nodes = []
            for result in results:
                new_node_with_score = NodeWithScore(
                    node=nodes[result[0]].node, score=result[1]["relevance_score"]
                )
